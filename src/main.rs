...
impl<Iter: Iterator<Item = char> + Clone> Parser<Iter> {
    ...
    fn dvnumber(self) -> ParseResult<u32, Iter> {
        self.dvchar().and_then(|(c, deriv)| Self::ctoi(c).map(|i| (i, deriv)))
    }
            ...
            match deriv.clone().dvchar() {
                Ok(('*', deriv)) => {
                    match deriv.dvmultitive() {
                        Ok((rhs, deriv)) => {
                            Ok((Expr::Multiply(Box::new(lhs), Box::new(rhs)), deriv))
                        }
                        Err(_) => Err(()),
                    }
                }
        ...
        if let Ok((lhs, deriv)) = self.dvmultitive() {
            match deriv.clone().dvchar() {
                ...
                Ok(('-', deriv)) => {
                    match deriv.dvadditive() {
                        Ok((rhs, deriv)) => {
                            Ok((Expr::Subtract(Box::new(lhs), Box::new(rhs)), deriv))
                        }
                        Err(_) => Err(()),
                    }
                }
